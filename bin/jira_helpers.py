...
def update_jira_settings(jira_settings, server_uri, session_key):
    r = requests.post(
        ...
        verify=False).json()
    requests.post(
        url=server_uri + '/servicesNS/nobody/SA-alert_manager-jira/storage/passwords/%3Ajira_password%3A?output_mode=json',
        data={
...
def get_jira_password(server_uri, session_key):
    password_url = server_uri + '/servicesNS/nobody/SA-alert_manager-jira/storage/passwords/%3Ajira_password%3A?output_mode=json'
